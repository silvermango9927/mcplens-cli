...
export function activationJsonTemplate(manifest: Manifest, outDir: string): string {
  const serverName = slug(manifest.api.name)
  const entrypoint = generatedEntrypoint(outDir)
  const authEnv = authEnvVars(manifest.api.auth)
  return JSON.stringify(
    {
      agentifyActivationVersion: 1,
      serverName,
      displayName: `${manifest.api.name} MCP`,
      projectDir: path.resolve(outDir),
      entrypoint,
      stdio: {
        command: 'node',
        args: [entrypoint],
        env: placeholderEnv(authEnv)
      },
      claudeCode: {
        command: claudeCodeCommand(serverName, entrypoint, authEnv)
      },
      streamableHttp: {
        url: 'http://127.0.0.1:3000/mcp',
        healthUrl: 'http://127.0.0.1:3000/healthz',
        requiredEnv: ['MCP_TRANSPORT=http', 'MCP_HTTP_TOKEN=<token>', ...authEnv.map((name) => `${name}=<value>`)]
      },
      notes: [
        'Run npm install && npm run build in the generated project before activating stdio clients.',
        'Replace every <...> placeholder with a real credential before starting the server.',
        'Use Streamable HTTP only behind TLS plus a trusted firewall or reverse proxy when exposing it beyond localhost.'
      ]
    },
    null,
    2
...
export function mcpClientConfigTemplate(manifest: Manifest, outDir: string): string {
  const serverName = slug(manifest.api.name)
  return JSON.stringify(
    {
      mcpServers: {
        [serverName]: {
          command: 'node',
          args: [generatedEntrypoint(outDir)],
          env: placeholderEnv(authEnvVars(manifest.api.auth))
        }
      }
    },
    null,
...
export function activationGuideTemplate(manifest: Manifest, outDir: string): string {
  const serverName = slug(manifest.api.name)
  const entrypoint = generatedEntrypoint(outDir)
  const authEnv = authEnvVars(manifest.api.auth)
  return `# Activate ${manifest.api.name} MCP

This file is generated by agentify so the MCP client configuration points at this exact
build output.

## 1. Build The Server

\`\`\`sh
cd ${shellQuote(path.resolve(outDir))}
npm install
npm run build
\`\`\`
...
## 3. Activate In A Local MCP Client

Use \`mcp-client.config.json\` for clients that accept an \`mcpServers\` JSON object, such
as Claude Desktop and Cursor. Replace every placeholder value before restarting the
client.

\`\`\`json
${mcpClientConfigTemplate(manifest, outDir)}
\`\`\`

Claude Code can add the server directly:

\`\`\`sh
${claudeCodeCommand(serverName, entrypoint, authEnv)}
\`\`\`

The command launches the generated server over stdio. Stdio keeps API credentials local
to the MCP client process and does not open a network port.

## 4. Optional Streamable HTTP

For self-hosted or remote clients, run the generated server in HTTP mode:
...
The MCP endpoint is \`http://127.0.0.1:3000/mcp\`. Clients must send:

\`\`\`http
Authorization: Bearer $MCP_HTTP_TOKEN
\`\`\`

Do not expose \`/mcp\` beyond localhost without TLS, a strong token, and a trusted
...
function generatedEntrypoint(outDir: string): string {
  return path.resolve(outDir, 'dist/index.js')
}

function placeholderEnv(names: string[]): Record<string, string> {
  return Object.fromEntries(names.map((name) => [name, `<${name}>`]))
}

function claudeCodeCommand(serverName: string, entrypoint: string, envVars: string[]): string {
  const envArgs = envVars.map((name) => `--env ${name}="$${name}"`)
  return ['claude', 'mcp', 'add', shellQuote(serverName), ...envArgs, '--', 'node', shellQuote(entrypoint)].join(' ')
}

...
Generated by agentify from the ${manifest.api.name} API.
...
| --- | --- | --- | --- |
| \`MCP_TRANSPORT\` | No | \`stdio\` | Use \`stdio\` for local MCP clients or \`http\` for Streamable HTTP. |
| \`HOST\` | HTTP only | \`127.0.0.1\` | Bind address for HTTP mode. Use \`0.0.0.0\` only behind a trusted firewall or reverse proxy. |
| \`PORT\` | HTTP only | \`3000\` | HTTP listen port. |
| \`MCP_HTTP_TOKEN\` | HTTP only | None | Bearer token required for every \`/mcp\` request in HTTP mode. |
| \`AGENTIFY_BASE_URL\` | No | \`${manifest.api.baseUrl}\` | Upstream API base URL. |
${authEnvRows(manifest.api.auth)}

Copy \`.env.example\` to \`.env\` when running with Docker Compose or when you prefer
loading env vars from a shell profile. Replace every placeholder value before exposing
the server to another machine.

## Stdio Mode

Stdio is the default transport and is the right choice when your MCP client launches
this project as a local command.

\`\`\`sh
${authShellExports(manifest.api.auth)}
npm start
\`\`\`

Connect an MCP client with a command similar to:

\`\`\`json
{
  "mcpServers": {
    "${slug(manifest.api.name)}": {
      "command": "node",
      "args": ["/absolute/path/to/${slug(manifest.api.name)}-mcp/dist/index.js"],
      "env": ${stdioClientEnvJson(manifest)}
    }
  }
}
\`\`\`
...
HTTP mode serves MCP at \`/mcp\` and exposes unauthenticated health probes at
\`/healthz\` and \`/readyz\`.
...
Clients must send the bearer token on every MCP request:

\`\`\`http
Authorization: Bearer $MCP_HTTP_TOKEN
\`\`\`

Do not expose \`/mcp\` on the public internet without a strong \`MCP_HTTP_TOKEN\`, TLS,
and a reverse proxy or firewall that you control. The token protects the MCP endpoint;
...
Compose runs HTTP mode on port 3000 by default:
...
function authShellExports(auth: ManifestAuth): string {
  return authInstructions(auth).replaceAll('...', 'replace-me')
}

function authEnvRows(auth: ManifestAuth): string {
  if (auth.type === 'bearer') {
    return `| \`${auth.envVar}\` | Yes | None | Upstream bearer token for ${auth.envVar}. |`
  }
  if (auth.type === 'header') {
    return `| \`${auth.envVar}\` | Yes | None | Upstream value sent as the \`${auth.header}\` header. |`
  }
  if (auth.type === 'basic') {
    return [
      `| \`${auth.userEnvVar}\` | Yes | None | Upstream basic auth username. |`,
      `| \`${auth.passEnvVar}\` | Yes | None | Upstream basic auth password. |`
    ].join('\n')
  }
  return '| Upstream auth | No | None | This manifest did not infer upstream credentials. |'
}

function stdioClientEnvJson(manifest: Manifest): string {
  const env: Record<string, string> = Object.fromEntries(authEnvVars(manifest.api.auth).map((name) => [name, `\${${name}}`]))
  if (Object.keys(env).length === 0) return '{}'
  return JSON.stringify(env, null, 8)
}
